...
var regCardMsg = '카드 번호를 정확하게 입력해주세요'
...
$('#btn_charge').on('click', function(){
    ...
    $("#in_amt").val( $("#inp_point").val() );
    $("#in_vbankBankCode").val( $("#vbank_name option:selected").val() );
    $("#in_receiptAmt").val( $("#inp_point").val() );
    if($("#inp_cash_receipt").is(":checked") == true){ // 현금영수증 신청
        $("#in_receiptType").val( $(':radio[name="radio_cash_receipt"]:checked').val() );
        if( $(':radio[name="radio_cash_receipt"]:checked').val() == "1" ) { // 소득공제
            $("#in_receiptIdentity").val($("#certi_cash_receipt").val());
        } else if( $(':radio[name="radio_cash_receipt"]:checked').val() == "2" ) { // 지출증빙
            $("#in_receiptIdentity").val($("#certi_cash_receipt2").val());
        }
    } else {
        $("#in_receiptType").val( "1" );
        $("#in_receiptIdentity").val("0100001234"); // 자진발급
    }
